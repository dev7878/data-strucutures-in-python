...
class ByLetter:
    ...
    def __lt__(self, target):
        ...
        # Your code here
        if ord(self.letter) < ord(target.letter):
            result = True
        else:
            result = False
        return result
...
def encode_morse(bst, text):
    ...
    for letter in text:
        if letter.isalpha():
            key = ByLetter(letter.upper(), None)
            value = bst.retrieve(key).code
